    ...
    <button className={Btn_container}>
      {loading || !allowLiking ? (
        <div className={Btn_wrapper}>
          <input disabled type="checkbox" id="like_id" name="check" checked={false} onChange={() => {}} />
          ...
          <div className={particles} style={{ '--total-particles': 6 } as myCustomCSS}>
            <div className={particle} style={{ '--i': 1, '--color': '#7642F0' } as myCustomCSS}></div>
            <div className={particle} style={{ '--i': 2, '--color': '#AFD27F' } as myCustomCSS}></div>
            <div className={particle} style={{ '--i': 3, '--color': '#DE8F4F' } as myCustomCSS}></div>
            <div className={particle} style={{ '--i': 4, '--color': '#D0516B' } as myCustomCSS}></div>
            <div className={particle} style={{ '--i': 5, '--color': '#5686F2' } as myCustomCSS}></div>
            <div className={particle} style={{ '--i': 6, '--color': '#D53EF3' } as myCustomCSS}></div>
          </div>
        </div>
      ) : (
        <div className={Btn_wrapper}>
      ...
      <span className={`${likeCountContainer} ${showCount ? showLikeCount : ''}`}>
        {loading || !allowLiking ? <span className={styles.loadingDot}>·</span> : likeCount}
      </span>
    </button>
  )
}
