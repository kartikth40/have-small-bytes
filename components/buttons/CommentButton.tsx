...
export default function CommentButton({
  ...
  return (
    <button className={`${Btn_container} ${commentBtn}`}>
      <div className={Btn_wrapper}>
        {onPage ? (
          <a href={`#comment-section-${postId}`}>
            <svg className={comment} width="24" height="24" viewBox="0 0 50 50">
      ...
      <span className={`${commentCountContainer} ${showCount ? showCommentCount : ''}`}>
        {loading ? <span className={styles.loadingDot}>·</span> : commentsCount}
      </span>
    </button>
  )
}
